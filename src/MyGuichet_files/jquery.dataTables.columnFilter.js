    ...
    $.fn.columnFilter = function (options) {
        ...
        function _fnColumnIndex(iColumnIndex) {
            if (properties.bUseColVis)
				...
				// CTIE : added test on fnSettings if (oTable.fnSettings()) 
                if (oTable.fnSettings()) return oTable.fnSettings().oApi._fnVisibleToColumnIndex(oTable.fnSettings(), iColumnIndex);
            //return iColumnIndex;
        ...
        function fnCreateColumnSelect(oTable, aData, iColumn, nTh, sLabel, bRegex) {
            ...
            for (j = 0; j < iLen; j++) {
                if (typeof (aData[j]) != 'object') {
                    ...
                    /* CTIE : strip html tags from string
					 * original : r += '<option ' + selected + ' value="' + escape(aData[j]) + '">' + aData[j] + '</option>';
					 */
                    r += '<option ' + selected + ' value="' + escape(aData[j].replace(/<(?:.|\n)*?>/gm, '')) + '">' + aData[j] + '</option>';
                }
                else {
                    var selected = '';
                    if (bRegex) {
