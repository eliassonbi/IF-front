/*!
* jsTree 1.9.1r http://jstree.com/
* ========================================================================
* add tree to '.doc-tree' lists and handle menu-boutons
*
* Revision: @wildmotion (see CTIE additions)
* 
...
* 
* $Date: 2010-07-01 10:51:11 +0300 (—á–µ—Ç–≤, 01 —é–ª–∏ 2010) $ $Revision: 191r $
*/
...
//All styles present here have been deactivated. Search for those to reactivate /*$.vakata.css.add_sheet({ str : css_string });*/

/**************************************************
 * CTIE additions to core script
 **************************************************
 Added functionality for use with sublists that
 are not to be considered part of the tree

 - only use .jstree-link for nod handeling ignore other links 
			search for .jstree-link
 - ignore non tree lists 
			search for .jstree-ignore
	...
	$.jstree.plugin("core", {
		...
		_fn : { 
			...
			_get_next		: function (obj, strict) {
				...
				/* CTIE : 	added "> ul > " before li so it only selects tree elements 
							and not subLists used for other purposes, like : action menu */
				if(obj.hasClass("jstree-open")) { return obj.find("> ul > li:eq(0)"); }
			...
			open_node	: function (obj, callback, skip_animation) {
				...
				obj.children('a.jstree-link').eq(0)
				...
				if(!this._is_loaded(obj)) {
					obj.children('a.jstree-link').addClass("jstree-loading");
					this.load_node(obj, function () { t.open_node(obj, callback, skip_animation); }, callback);
				}
				else {
					if(s) { obj.children("ul").css("display","none"); }
					obj.removeClass("jstree-closed").addClass("jstree-open").children('a.jstree-link').removeClass("jstree-loading");
					if(s) { obj.children("ul").stop(true).slideDown(s, function () { this.style.display = ""; t.after_open(obj); }); }
			...
			close_node	: function (obj, skip_animation) {
				...
				obj.not($('.jstree-ignore li')).removeClass("jstree-open").addClass("jstree-closed");
				if(s) { obj.children("ul").stop(true).slideUp(s, function () { this.style.display = ""; t.after_close(obj); }); }
				this.__callback({ "obj" : obj });
			},
			...
			clean_node	: function (obj) {
				...
				obj.removeClass("jstree-last")
					.filter("li:last-child:not(.jstree-ignore li)").addClass("jstree-last").end()
					.filter(":has(> ul:not(.jstree-ignore) > li)")
						.not(".jstree-open").removeClass("jstree-leaf").not($('.jstree-ignore li')).addClass("jstree-closed");
				obj.not(".jstree-open, .jstree-closed").not($('.jstree-ignore li')).addClass("jstree-leaf").children("ul").remove();
				this.__callback({ "obj" : obj });
			},
			// rollback
			...
			get_text	: function (obj) {
				...
				obj = obj.children("a.jstree-link:eq(0)");
				if(s) {
					obj = obj.clone();
...
(function ($) {
	$.jstree.plugin("ui", {
		__init : function () { 
			...
			this.get_container()
				.delegate("a.jstree-link", "click.jstree", $.proxy(function (event) {
						event.preventDefault();
						this.select_node(event.currentTarget, true, event);
					}, this))
				.delegate("a.jstree-link", "mouseenter.jstree", $.proxy(function (event) {
						this.hover_node(event.target);
					}, this))
				.delegate("a.jstree-link", "mouseleave.jstree", $.proxy(function (event) {
		...
		_fn : { 
			...
			hover_node : function (obj) {
				...
				this.data.ui.hovered = obj.children('a.jstree-link').addClass("jstree-hovered").parent();
				this.__callback({ "obj" : obj });
			},
			dehover_node : function () {
			    var obj = this.data.ui.hovered, p;
				if(!obj || !obj.length) { return false; }
				p = obj.children('a.jstree-link').removeClass("jstree-hovered").parent();
				if(this.data.ui.hovered[0] === p[0]) { this.data.ui.hovered= null; }
				this.__callback({ "obj" : obj });
			},
			select_node : function (obj, check, e) {
			    $(obj).find('a.jstree-link').eq(0)
			        .attr("tabindex", "0")
				...
				if(proceed && !is_selected) {
					obj.children('a.jstree-link').addClass("jstree-clicked");
			...
			deselect_node : function (obj) {
				...
				if(this.is_selected(obj)) {
					...
					obj.children('a.jstree-link').removeClass("jstree-clicked");
		...
		_fn : {
			_show_input : function (obj, callback) {
				...
				var rtl = this._get_settings().core.rtl,
					...
					w2 = obj.find("> a.jstree-link:visible > ins").width() * obj.find("> a.jstree-link:visible > ins").length,
					t = this.get_text(obj),
...
(function ($) {
	...
	$.jstree.plugin("hotkeys", {
		__init : function () {
			...
			$(this.data.html_data.original_container_html).find("a.jstree-link").attr("tabindex", "-1").eq(0).attr("tabindex", "0");
			$.each(this._get_settings().hotkeys, function (i, val) {
				if($.inArray(i, bound) == -1) {
					/* CTIE : added find("a.jstree-link") to keep the keydown action only on specific links */
				    $(self.data.html_data.original_container_html).find('a').not($('.jstree-ignore a')).bind("keydown", i, function (event) { return exec(i, event); });
					bound.push(i);
				}
			});
			this.enable_hotkeys();
		...
		defaults : {
			...
			"right" : function () { 
				var o = this.data.ui.hovered || this.data.ui.last_selected;
				if(o && o.length && o.not($('.jstree-ignore li'))) {
			...
			"space" : function () { 
				...
				/* CTIE replaced the following line with code above
			...
			"ctrl+space" : function (event) { 
				...
				var o = this.data.ui.hovered || this.data.ui.last_selected;
				if(o && o.length) {
					o.find("a.jstree-link").trigger(event);
				}
				/* CTIE replaced the following line with code above
	...
	$.jstree.plugin("languages", {
		...
		_fn : {
			...
			set_text : function (obj, val, lang) {
				...
				else { obj = obj.children("a.jstree-link:eq(0)"); }
				if(s) {
	...
	$.jstree.plugin("dnd", {
		__init : function () {
			...
			this.get_container()
				...
				.delegate("a.jstree-link", "mousedown.jstree", $.proxy(function (e) { 
						if(e.which === 1) {
							this.start_drag(e.currentTarget, e);
							return false;
						}
					}, this))
				.delegate("a.jstree-link", "mouseenter.jstree", $.proxy(function (e) { 
						if($.vakata.dnd.is_drag && $.vakata.dnd.user_data.jstree) {
							this.dnd_enter(e.currentTarget);
						}
					}, this))
				.delegate("a.jstree-link", "mousemove.jstree", $.proxy(function (e) { 
						if($.vakata.dnd.is_drag && $.vakata.dnd.user_data.jstree) {
				...
				.delegate("a.jstree-link", "mouseleave.jstree", $.proxy(function (e) { 
						if($.vakata.dnd.is_drag && $.vakata.dnd.user_data.jstree) {
				...
				.delegate("a.jstree-link", "mouseup.jstree", $.proxy(function (e) { 
						if($.vakata.dnd.is_drag && $.vakata.dnd.user_data.jstree) {
							this.dnd_finish(e);
						}
					}, this));
	...
	$.jstree.plugin("checkbox", {
		__html : function () {
			...
			this.get_container()
				...
				.delegate("a.jstree-link", "click.jstree", $.proxy(function (e) {
		...
		_fn : {
			_prepare_checkboxes : function (obj) {
				...
				obj.each(function () {
					...
					t.find("a.jstree-link").not(":has(.jstree-checkbox)").prepend("<ins class='jstree-checkbox'> </ins>").parent().not(".jstree-checked, .jstree-unchecked").addClass(c);
				});
	...
	$.jstree.plugin("search", {
		...
		_fn : {
			search : function (str, skip_async) {
				...
				this.data.search.result = this.get_container().find("a.jstree-link" + (this.data.languages ? "." + this.get_lang() : "" ) + ":" + (s.case_insensitive ? "jstree_contains" : "contains") + "(" + this.data.search.str + ")");
...
(function ($) {
	...
	$(function () {
		...
		/*$.vakata.css.add_sheet({ str : css_string });*/
		$.vakata.context.cnt
			.delegate("a.jstree-link","click", function (e) { e.preventDefault(); })
			.delegate("a.jstree-link","mouseup", function (e) {
				if(!$(this).parent().hasClass("jstree-contextmenu-disabled") && $.vakata.context.exec($(this).attr("rel"))) {
					$.vakata.context.hide();
				}
				else { $(this).blur(); }
			})
			.delegate("a.jstree-link","mouseover", function () {
		...
		if(typeof $.hotkeys !== "undefined") {
			$(document)
				...
				.bind("keydown", "space", function (e) { 
					$.vakata.context.cnt.find(".vakata-hover").last().children("a.jstree-link").click();
					e.preventDefault();
				});
		}
	});
	...
	$.jstree.plugin("contextmenu", {
		__init : function () {
			this.get_container()
				.delegate("a.jstree-link", "contextmenu.jstree", $.proxy(function (e) {
						e.preventDefault();
				...
				switch(!0) {
					case (!s.data && !s.ajax):
						if(!obj || obj == -1) {
							this.get_container()
								...
								.find("li").not($('.jstree-ignore li')).filter(function () { return this.firstChild.tagName !== "INS"; }).prepend("<ins class='jstree-icon'> </ins>")
								.find("a").not($('.jstree-ignore a')).filter(function () { return this.firstChild.tagName !== "INS"; }).prepend("<ins class='jstree-icon'> </ins>").end()
								.filter("a").not($('.jstree-ignore a')).children("ins:first-child").not(".jstree-icon").addClass("jstree-icon");
						    
							
							//CTIE : check if the class .jstree-link is specified otherwise add it
							var jsLinkLength = this.get_container().find(".jstree-link").length;
							if (jsLinkLength == 0) {
								this.get_container().find("li > a").addClass("jstree-link");
							}
						    this.get_container()
                                .find("ul:eq(0)")
                                .attr("role", "tree")
                                .end()
                                .find("ul").not($('.jstree-ignore, .jstree-ignore ul'))
                                    .attr("role", "group")
                                .find("li")
						    ...
						    this.get_container().find(".jstree-closed .jstree-link").not($('.jstree-ignore li .jstree-link'))
	                            .attr("aria-expanded", "false");
						    this.get_container().find(".jstree-open .jstree-link").not($('.jstree-ignore li .jstree-link'))
    	                        .attr("aria-expanded", "true");
					...
					case (!!s.data && !s.ajax) || (!!s.data && !!s.ajax && (!obj || obj === -1)):
						if(!obj || obj == -1) {
							...
							this.get_container()
								.children("ul").not($('.jstree-ignore')).empty().append(d.children())
								.find("li").not($('.jstree-ignore li')).filter(function () { return this.firstChild.tagName !== "INS"; }).prepend("<ins class='jstree-icon'> </ins>").end()
								.find("a").not($('.jstree-ignore a')).filter(function () { return this.firstChild.tagName !== "INS"; }).prepend("<ins class='jstree-icon'> </ins>").end()
								.filter("a.jstree-link").children("ins:first-child").not(".jstree-icon").addClass("jstree-icon");
							this.clean_node();
						}
						if(s_call) { s_call.call(this); }
						break;
					case (!s.data && !!s.ajax) || (!!s.data && !!s.ajax && obj && obj !== -1):
						...
						error_func = function (x, t, e) {
							...
							if(obj != -1 && obj.length) {
								obj.children(".jstree-loading").removeClass("jstree-loading");
								obj.data("jstree-is-loading",false);
								if(t === "success" && s.correct_state) { obj.removeClass("jstree-open jstree-closed").not($('.jstree-ignore li')).addClass("jstree-leaf"); }
							}
							else {
								if(t === "success" && s.correct_state) { this.get_container().children("ul").empty(); }
							}
						...
						success_func = function (d, t, x) {
							...
							if(d) {
								d = $(d);
								if(!d.is("ul")) { d = $("<ul>").append(d); }
								if(obj == -1 || !obj) { this.get_container().children("ul").empty().append(d.children()).find("li, a").filter(function () { return this.firstChild.tagName !== "INS"; }).prepend("<ins class='jstree-icon'> </ins>").end().filter("a.jstree-link").children("ins:first-child").not(".jstree-icon").addClass("jstree-icon"); }
								else { obj.children(".jstree-loading").removeClass("jstree-loading"); obj.append(d).find("li, a").filter(function () { return this.firstChild.tagName !== "INS"; }).prepend("<ins class='jstree-icon'> </ins>").end().filter("a.jstree-link").children("ins:first-child").not(".jstree-icon").addClass("jstree-icon"); obj.data("jstree-is-loading",false); }
								this.clean_node(obj);
								if(s_call) { s_call.call(this); }
							}
							else {
								if(obj && obj !== -1) {
									...
									obj.data("jstree-is-loading",false);
									if(s.correct_state) { 
										obj.removeClass("jstree-open jstree-closed").not($('.jstree-ignore li')).addClass("jstree-leaf"); 
										if(s_call) { s_call.call(this); } 
									}
								}
								else {
	...
	$.jstree.plugin("themeroller", {
		__init : function () {
			...
			this.get_container()
				...
				.delegate("a.jstree-link","mouseenter.jstree", function () {
					$(this).addClass(s.item_h);
				})
				.delegate("a.jstree-link","mouseleave.jstree", function () {
				...
				.bind("select_node.jstree", $.proxy(function (e, data) {
						data.rslt.obj.children("a.jstree-link").addClass(s.item_a);
					}, this))
		...
		_fn : {
			_themeroller : function (obj) {
				...
				obj
					...
					.find("a.jstree-link").addClass(s.item)
	...
	$.jstree.plugin("unique", {
						...
						if(data.func == "move_node") {
							...
							if(data.args[4] === true) {
								if(data.args[0].o && data.args[0].o.length) {
									data.args[0].o.children("a.jstree-link").each(function () { nms.push($(this).text().replace(/^\s+/g,"")); });
		...
		_fn : { 
			_check_unique : function (nms, p) {
				var cnms = [];
				p.children("a.jstree-link").each(function () { cnms.push($(this).text().replace(/^\s+/g,"")); });
			...
			check_move : function () {
				...
				if(p.o && p.o.length) {
					p.o.children("a.jstree-link").each(function () { nms.push($(this).text().replace(/^\s+/g,"")); });
